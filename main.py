...
def init_db():
    ...
    if conn.execute("SELECT COUNT(*) c FROM prizes").fetchone()["c"] == 0:
        conn.executemany(
    ...
    if conn.execute("SELECT COUNT(*) c FROM settings WHERE key='referral_amount'").fetchone()["c"] == 0:
...
async def api_stats(request):
    ...
    total_spins = conn.execute("SELECT COUNT(*) c FROM spins").fetchone()["c"]
    prize_breakdown = conn.execute(
    ...
    return web.json_response({
        ...
        "prize_breakdown": [dict(p) for p in prize_breakdown],
        "referral_pending_count": ref_pending["c"],
        "referral_pending_sum": ref_pending["s"],
        "referral_paid_count": ref_paid["c"],
        "referral_paid_sum": ref_paid["s"],
    }, headers=CORS_HEADERS)
...
def main():
    ...
    options_paths_added = set()
    for method, path, handler in routes:
        app.router.add_route(method, path, handler)
        if path not in options_paths_added:
            app.router.add_route("OPTIONS", path, options_handler)
            options_paths_added.add(path)
